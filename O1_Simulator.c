			...
			for(k = 0; k < n; k += bk){
				for(ib = i; (ib < i + bi)&&(ib < n); ib++){
 					for(jb = j; (jb < j + bj)&&(jb < n); jb++) {
						...
						for(int ib1 = i; (ib1 < i + bi)&&(ib1 < n); ib1++)
 							for(int jb1 = j; (jb1 < j + bj)&&(jb1 < n); jb1++) 
								cache_functionLRU ( M, &global_time, 1, Cid[ib1][jb1], id_Array, timestamps, write_or_no, nextyounger, nextolder, &oldest, &youngest, &read, &write, index_in_cache);
				}
...
for (int i = 0; i < n; i++) {
	...
	printf("read: %d, write: %d, io: %d, elapsed: %f\n", read, write, io, elapsed);
