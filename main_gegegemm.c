...
//int cache_functionMRU (int M, int *time, int ischanged, int id, 
...
//						int *read, int *write) 
			int cache_functionLRU (int M, int *time, int ischanged, int id, 
				...
				if (itisincache == 0) {
					...
					//find oldest_time = smallest time.
					// if (changed[oldest_loc] == 1) { (*write)++; printf("writing back %8d to put %8d!\n",isincache[oldest_loc],id); }
					if (changed[oldest_loc] == 1) (*write)++;
					(*read)++;
					//swapping oldest_loc for all three arrays
					isincache[oldest_loc] = id;
